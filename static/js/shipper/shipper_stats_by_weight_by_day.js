function drawShipperStatsByWeightByDay() {
    const source_data = JSON.parse(window.shipper_merchandise_by_weight_by_day);
    ...
    for (i = 0; i < source_data.length; i++) {
        ...
        for (let j = 0; j < node_data.length; j++) {
            ...
            issuers_data[issuerName].push(node_data[j]["peso"])
        }
    }
    ...
    const ctx = document.getElementById('shipper-stats-by-weight-by-day-chart-container').getContext('2d');
    window.myBar = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Peso diario por cada transportista'
                }
            },
        }
    });
